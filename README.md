...
- [Сетевая архитектура](docs/network.md) — компоненты, типы сообщений, жизненный цикл соединения
- [Lua API](docs/lua-api.md) — справочник по встроенным функциям и методам
- [Примеры и сниппеты](docs/examples/README.md) — готовые игры и изолированные примеры с кодом

## Примеры игр

- **kosti** — Игра в кости (одиночная / сетевая)
  ```bash
  ./dice scenes/kosti.json                        # одиночная
  ./dice scenes/kosti.json --host 7777            # хост
  ./dice scenes/kosti.json --join 192.168.1.5:7777  # клиент
  ```
