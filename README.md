...
- Обязывает пройти весь воркфлоу перед написанием кода: дизайн-документ → план → реализация
- Содержит полный справочник по Lua API, типам объектов, системе событий и пресетам
- Хранит готовые паттерны (drag-and-drop, deck draw, turn-based state, таймеры)
- Документирует все ограничения движка с обходными решениями

Скилл подключается автоматически при работе с файлами `.json` сцен, Lua-скриптами и `game.json`.
